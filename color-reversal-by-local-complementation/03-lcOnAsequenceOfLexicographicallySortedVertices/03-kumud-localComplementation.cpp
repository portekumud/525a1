...
vector<int> h10_checkEquivalentGraphStructure(vector<vector<vector<int>>> theSetOfAllGraphs, vector<vector<int>> theSetOfAllBicolorations, vector<int> &resultGraphEq, vector<int> &resultBicolorEq, vector<string> setOfAllLexico, int);
...
inline void kumudLoggerThinkPad( string logMsg ) {
    ...
    string filePath = "../log/logfile03Kumud_" + getCurrentDateTime("date")+"-";
...
int main(int argc, char **argv) {   
	...
	if( argc == 3 ) {
		...
		if(validORnotArgumentOneTh == IsValid && validORnotArgumentTwoThForCode3 == IsValid) {
			...
			finalEquivalence = h10_checkEquivalentGraphStructure(theSetOfAllGraphs, theSetOfAllBicolorations, resultGraphEq, resultBicolorEq, setOfAllLexico, stoi(*(argv + 2)));
		}
		else{
		...
		cout << "The number of arguments is not 3. Exiting. Try again: " << argc;
	}
...
std::chrono::duration<double> elapsed_seconds = end-start;
    ...
    cout << "Creating images and preparting output file" << endl;
    return 0;
}
...
void h01_readTheInputFileAndRecoverTheTwoHiddenThings(char *myInputFileName, vector<vector<int>> &myHiddenGraphInTheInputFile, vector<int> &myHiddenBiColorationInTheInputFile) {
	...
	cout << endl << endl << "In h01_readTheInputFileAndRecoverTheTwoHiddenThings. " << myInputFileName << endl;
...
void h09_drawPNGorPDF(vector<vector<int>> myGraph, vector<int> myBicoloration, string parameter1, string parameter2) {
	...
	string name = "../allFilesPython/graph" + parameter1 + "-" + parameter2 + ".py";
...
vector<int> h10_checkEquivalentGraphStructure(vector<vector<vector<int>>> theSetOfAllGraphs, vector<vector<int>> theSetOfAllBicolorations, vector<int> &resultGraphEq, vector<int> &resultBicolorEq, vector<string> setOfAllLexico, int upto) {
	//theAnswerIsNo
	...
	string name = "../output/result" ;
	ofstream file(name, std::ios_base::out);

	file << "This file contains the positive results of color inversion using all lexicographically strings up to length " << upto << endl;
	...
	for(x2 = resultGraphEq.begin() + 1, x3 =resultBicolorEq.begin() + 1, hj = 0, i1_setOfAllLexico = setOfAllLexico.begin(), counter = 0; x2 != resultGraphEq.end(); x2++, x3++, hj++, i1_setOfAllLexico++, counter++) {
		if(*x2 == 93 && *x3 == 45) {
			...
			file << "Color inversion is achieved using the string " << *i1_setOfAllLexico << ". The array index is " << counter << endl;
